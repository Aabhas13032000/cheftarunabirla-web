    ...
    pool.query(user,function(err,user,fields){
        if(err) {
            ...
            if(user.length !=0){
                ...
                const featured_courses = "SELECT c.* , (SELECT `path` FROM `images` WHERE `images`.`course_id` = c.`id` AND `iv_category` = 'image' LIMIT 1 OFFSET 0) AS image_path ,(SELECT COUNT(*) FROM `cart` WHERE `cart`.`course_id` = c.`id` AND `cart`.`user_id` = '"+ user[0].id +"') AS count,(SELECT COUNT(*) FROM `cart` WHERE `cart`.`course_id` = c.`id` AND `cart`.`cart_category` = 'whislist' AND `cart`.`user_id` = '"+ user[0].id +"') AS whislistcount,(SELECT COUNT(*) FROM `subscription` WHERE `subscription`.`course_id` = c.`id` AND `subscription`.`user_id` = '"+ user[0].id +"' AND `status` = 1) AS subscribed FROM `courses` c WHERE `status` = 1 AND `featured` = 1 ORDER BY `created_at` DESC LIMIT 4 OFFSET 0";
                ...
                pool.query(cart,function(err,cart,fields){
                    pool.query(product_categories,function(err,product_categories,fields){
                        pool.query(course_categories,function(err,course_categories,fields){
                            pool.query(user_course_subscription,function(err,user_course_subscription,fields){
                                pool.query(user_book_subscription,function(err,user_book_subscription,fields){
                                                    ...
                                                    pool.query(featured_products,function(err,featured_products,fields){
                                                        if(err) {
                                                            ...
                                                            res.json({
                                                                ...
                                                                user:user,
                                                                user_course_subscription:user_course_subscription,
                                                                user_book_subscription:user_book_subscription,
                                                                impBooks:impBooks,
                                                                slider:slider,
